...
5. 모르는 용어가 있으면 `needs_clarification` 필드를 true로 설정하고, 최선의 추측을 함께 반환해.
6. **요청이 모호하거나 정확히 어떤 항목인지 불확실하면, 관련된 데이터를 최대한 많이 추출해.** 예: "segment revenue"라고 하면 해당 세그먼트의 Revenue, Revenue % Change, Operating Profit, OPM 등 관련 지표를 함께 반환.
7. **회사명/티커**: 엑셀에서 회사명이나 티커를 최대한 찾아서 company 필드에 넣어. 시트명, 헤더, 파일 구조 등에서 힌트를 찾아.
